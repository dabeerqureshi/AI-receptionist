...
def show_login():
    ...
    st.markdown(
        '<div class="login-subtitle">AI Receptionist — Secure Admin Portal</div>',
        unsafe_allow_html=True,
    )
    ...
    if st.button("Sign In", type="primary", width="stretch"):
        try:
            ...
            logger.warning(
                "Failed login attempt %d for username: %s", attempts, username
            )
...
with st.sidebar:
    ...
    st.markdown(
        "<div style='margin-top:auto;padding-top:2rem;'>", unsafe_allow_html=True
    )
...
def plot_layout(height=280, **extra):
    ...
    base = dict(**PLOT_LAYOUT, height=height, xaxis=dict(**_AXIS), yaxis=dict(**_AXIS))
    base.update(extra)
    return base
...
if menu == "📊 Dashboard":
    st.markdown(
        '<div class="page-title">Overview Dashboard</div>', unsafe_allow_html=True
    )
    st.markdown(
        '<div class="page-subtitle">Real-time summary of your clinic network</div>',
        unsafe_allow_html=True,
    )
    ...
    cols = st.columns(4)
    with cols[0]:
        metric_card("Total Clinics", total_clinics, "🏥")
    with cols[1]:
        metric_card("All Appointments", total_appointments, "📅")
    with cols[2]:
        metric_card("Today's Bookings", today_count, "🗓️")
    with cols[3]:
        metric_card("This Week", week_count, "📆")

    st.markdown(
        '<div class="section-header">Recent Appointments</div>', unsafe_allow_html=True
    )

    if not df.empty:
        display_df = (
            df[["Clinic", "Patient", "Phone", "Date", "Time", "Reason"]].tail(15).copy()
        )
        display_df["Date"] = display_df["Date"].dt.strftime("%d %b %Y")
        st.dataframe(display_df, width="stretch", hide_index=True)
    else:
        st.markdown(
            '<div class="alert alert-info">ℹ️ No appointments recorded yet.</div>',
            unsafe_allow_html=True,
        )
    ...
    if not df.empty and df.shape[0] > 2:
        st.markdown(
            '<div class="section-header">Appointment Activity — Last 30 Days</div>',
            unsafe_allow_html=True,
        )
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        '<div class="page-subtitle">Appointment trends, clinic performance, and operational insights</div>',
        unsafe_allow_html=True,
    )
    ...
    if df.empty:
        st.markdown(
            '<div class="alert alert-warning">⚠️ Not enough data yet. Book some appointments to see analytics.</div>',
            unsafe_allow_html=True,
        )
        st.stop()
    ...
    if fdf.empty:
        st.markdown(
            '<div class="alert alert-info">ℹ️ No data for the selected filters.</div>',
            unsafe_allow_html=True,
        )
        st.stop()
    ...
    top_clinic = (
        fdf.groupby("Clinic").size().idxmax()
        if fdf["Clinic"].nunique() > 1
        else (fdf["Clinic"].iloc[0] if not fdf.empty else "—")
    )

    kcols = st.columns(4)
    with kcols[0]:
        metric_card("Appointments", total, "📅")
    with kcols[1]:
        metric_card("Unique Patients", unique_patients, "👤")
    with kcols[2]:
        metric_card("Busiest Day", busiest_day, "📆")
    with kcols[3]:
        metric_card("Top Clinic", top_clinic, "🏥")
    ...
    with r1c1:
        st.markdown(
            '<div class="section-header">Daily Appointments</div>',
            unsafe_allow_html=True,
        )
    ...
    with r1c2:
        st.markdown(
            '<div class="section-header">By Clinic</div>', unsafe_allow_html=True
        )
        by_clinic = fdf.groupby("Clinic").size().reset_index(name="Count")
        fig2 = go.Figure(
            go.Pie(
                labels=by_clinic["Clinic"],
                values=by_clinic["Count"],
                hole=0.55,
    ...
    with r2c1:
        st.markdown(
            '<div class="section-header">By Day of Week</div>', unsafe_allow_html=True
        )
        day_order = [
            "Monday",
            "Tuesday",
        ...
        by_dow = (
            fdf.groupby(fdf["Date"].dt.day_name())
            .size()
    ...
    with r2c2:
        st.markdown(
            '<div class="section-header">Appointment Time Distribution</div>',
            unsafe_allow_html=True,
        )
        try:
            fdf2 = fdf.copy()
            ...
            st.markdown(
                '<div class="alert alert-info">Time data unavailable for this chart.</div>',
                unsafe_allow_html=True,
            )
    ...
    if "Reason" in fdf.columns:
        st.markdown(
            '<div class="section-header">Top Appointment Reasons</div>',
            unsafe_allow_html=True,
        )
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        '<div class="page-subtitle">Create, configure and manage clinic tenants</div>',
        unsafe_allow_html=True,
    )
        ...
        with st.form("add_clinic_form", clear_on_submit=True):
            ...
            with c1:
                clinic_name = st.text_input(
                    "Clinic Name *", placeholder="e.g. Sunrise Family Clinic"
                )
            ...
            if submitted:
                if not clinic_name.strip():
                    st.markdown(
                        '<div class="alert alert-danger">❌ Clinic name is required.</div>',
                        unsafe_allow_html=True,
                    )
                else:
    ...
    st.markdown(
        '<div class="section-header">Registered Clinics</div>', unsafe_allow_html=True
    )
    clinics = get_all_clinics()

    if not clinics:
        st.markdown(
            '<div class="alert alert-info">ℹ️ No clinics yet. Create your first one above.</div>',
            unsafe_allow_html=True,
        )
    else:
        ...
        for clinic in clinics:
            ...
            appt_count = (
                df_appts[df_appts["clinic_id"] == clinic.id].shape[0]
            ...
            with st.container():
                ...
                with st.expander(f"Manage — {clinic.name}"):
                    ...
                    with tab1:
                        ...
                        st.markdown(
                            f'<div class="api-key-box">{clinic.api_key}</div>',
                            unsafe_allow_html=True,
                        )

                        if st.button("🔄 Rotate API Key", key=f"rotate_{clinic.id}"):
                            new_key = admin_rotate_api_key(
                                st.session_state.admin_token, clinic.id
                            )
                    ...
                    with tab2:
                        st.markdown(
                            '<div class="alert alert-danger">⚠️ Deleting a clinic removes ALL associated data permanently.</div>',
                            unsafe_allow_html=True,
                        )
                        ...
                        if st.button("❌ Delete Clinic", key=f"delete_{clinic.id}"):
                            if confirm.strip() == clinic.name:
                                ...
                                st.markdown(
                                    '<div class="alert alert-danger">❌ Name does not match. Deletion cancelled.</div>',
                                    unsafe_allow_html=True,
                                )
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        '<div class="page-subtitle">Set availability for each clinic per day of week</div>',
        unsafe_allow_html=True,
    )
    ...
    if not clinics:
        st.markdown(
            '<div class="alert alert-warning">⚠️ Create a clinic first.</div>',
            unsafe_allow_html=True,
        )
    else:
        ...
        st.markdown(
            '<div class="section-header">Configure Hours</div>', unsafe_allow_html=True
        )
        updated_hours = []

        header_cols = st.columns([2, 3, 3, 1])
        header_cols[0].markdown(
            '<small style="color:var(--text-muted)">DAY</small>', unsafe_allow_html=True
        )
        ...
        header_cols[2].markdown(
            '<small style="color:var(--text-muted)">CLOSE</small>',
            unsafe_allow_html=True,
        )
        ...
        for day_num, day_name in enumerate(days):
            ...
            with col4:
                enabled = st.checkbox(
                    "",
                    value=day_num in hours_map,
                    key=f"en_{selected_clinic_id}_{day_num}",
                    label_visibility="collapsed",
                )

            with col2:
                sv = (
                    hours_map[day_num].start_time
                    if day_num in hours_map
                    else time(9, 0)
                )
                start = st.time_input(
                    "Start",
                    value=sv,
                    key=f"s_{selected_clinic_id}_{day_num}",
                    disabled=not enabled,
                    label_visibility="collapsed",
                )
            with col3:
                ev = (
                    hours_map[day_num].end_time if day_num in hours_map else time(17, 0)
                )
                end = st.time_input(
                    "End",
                    value=ev,
                    key=f"e_{selected_clinic_id}_{day_num}",
                    disabled=not enabled,
                    label_visibility="collapsed",
                )
            ...
            if enabled:
                if start >= end:
                    st.markdown(
                        f'<div class="alert alert-danger">⚠️ {day_name}: close time must be after open time.</div>',
                        unsafe_allow_html=True,
                    )
                else:
        ...
        if st.button("💾  Save Working Hours", type="primary"):
            admin_update_working_hours(
                st.session_state.admin_token, selected_clinic_id, updated_hours
            )
            logger.info("Working hours updated for clinic: %s", selected_clinic_id)
            st.markdown(
                '<div class="alert alert-success">✅ Working hours saved.</div>',
                unsafe_allow_html=True,
            )
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        '<div class="page-subtitle">View and manage all patient bookings</div>',
        unsafe_allow_html=True,
    )
    ...
    if df.empty:
        st.markdown(
            '<div class="alert alert-info">ℹ️ No appointments booked yet.</div>',
            unsafe_allow_html=True,
        )
    else:
        # Filters
        ...
        with fc3:
            sort_by = st.selectbox(
                "Sort by", ["Date ↓", "Date ↑", "Patient A–Z", "Clinic"]
            )
        ...
        if search.strip():
            mask = fdf["Patient"].str.contains(search, case=False, na=False) | fdf[
                "Phone"
        ...
        st.markdown(
            f'<div style="font-size:0.8rem;color:var(--text-muted);margin:0.5rem 0;">'
            f"Showing {len(fdf)} of {len(df)} records</div>",
            unsafe_allow_html=True,
        )

        display_df = fdf[
            ["Clinic", "Patient", "Phone", "Date", "Time", "Reason"]
        ].copy()
        display_df["Date"] = display_df["Date"].dt.strftime("%d %b %Y")
        st.dataframe(display_df, width="stretch", hide_index=True)
        ...
        if st.session_state.get("confirm_delete_all"):
            st.markdown(
                '<div class="alert alert-danger">⚠️ This will permanently delete every appointment record.</div>',
                unsafe_allow_html=True,
            )
...
# ─────────────────────────────────────────────
    ...
    st.markdown(
        '<div class="page-subtitle">Health, configuration, and maintenance</div>',
        unsafe_allow_html=True,
    )

    # Status
    st.markdown(
        '<div class="section-header">Service Health</div>', unsafe_allow_html=True
    )
    sc1, sc2, sc3 = st.columns(3)
    with sc1:
        db_ok = True
        try:
            db_ok = admin_get_system_health(st.session_state.admin_token)[
                "database_connected"
            ]
        except Exception:
            db_ok = False
    ...
    # API Info
    st.markdown(
        '<div class="section-header">API Endpoints</div>', unsafe_allow_html=True
    )
    st.code(
    ...
    st.markdown(
        '<div class="alert alert-warning">⚠️ Destructive actions below. These cannot be undone.</div>',
        unsafe_allow_html=True,
    )
    ...
    with m1:
        if st.button("🔄 Reset with Sample Data"):
            try:
                ...
                st.markdown(
                    '<div class="alert alert-success">✅ Database reset with sample data.</div>',
                    unsafe_allow_html=True,
                )
            except ImportError:
                st.markdown(
                    '<div class="alert alert-danger">❌ setup_sample_data.py not found.</div>',
                    unsafe_allow_html=True,
                )
    ...
    if st.session_state.get("confirm_clear_all"):
        st.markdown(
            '<div class="alert alert-danger">⚠️ This will wipe every clinic, appointment, and setting.</div>',
            unsafe_allow_html=True,
        )
        yc, nc = st.columns(2)
        with yc:
